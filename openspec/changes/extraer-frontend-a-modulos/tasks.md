...
- [x] 6.7 Login con Google end-to-end: **Pablo lo confirmó en staging el 2026-09-17**, junto con los atajos de teclado. Verificado además que `/auth/login` responde 302 a `accounts.google.com` y que `connect-src` conserva los orígenes de Google. **El flujo completo (consentimiento → callback → sesión) no se puede probar sin una cuenta real: queda para que Pablo lo confirme en staging.** Nota: el login es una navegación, no un fetch, así que el CSP no la gobierna — `connect-src` solo aplicaría a llamadas XHR a Google, que el frontend no hace
