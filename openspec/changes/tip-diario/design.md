...
**Non-Goals:**
- ~~Sincronizar el avance entre dispositivos~~ → **pasó a ser un objetivo el 2026-09-11**: el avance se guarda en la cuenta. Ver la decisión más abajo.
...
- **El avance vive en la cuenta, en dos columnas de `users`** (decisión de Pablo, 2026-09-11): `tip_index` y `tip_date`. La regla es la misma que se había pensado para `localStorage` —si la fecha guardada no es hoy, avanzar el índice (con vuelta al inicio al llegar al final) y grabar hoy; si es hoy, mostrar el índice tal cual—, pero el estado es de la persona y no del navegador. Eso cumple "un tip por día", "estable durante la jornada" y "no se pierde ninguno", y además hace que la progresión sea la misma desde el teléfono y desde la computadora, que es la premisa de una secuencia pedagógica.
  - *Por qué sale barato*: `/api/me` ya hace `SELECT ... FROM users WHERE email = ?` en cada carga del tablero (`src/routes/users.js:15`), así que leer el avance **no agrega una sola query**. Escribirlo es un `UPDATE` una vez por persona por día. Es un costo despreciable frente al hallazgo de overhead de D1 ya registrado en el backlog.
  - *Dónde vive la regla*: en el backend, junto al endpoint, y se testea con Vitest como el resto de la API. Esto evita el problema de testear lógica que vive inline en `index.html` — hoy `test/labels.spec.js` termina copiando la constante que quiere verificar.
  - *Sin endpoint nuevo*: el avance se resuelve dentro de `GET /api/me`, que el frontend llama una sola vez al cargar el tablero (`public/index.html:1513`) y no en el polling de 5s. Si `tip_date` no es hoy, el handler avanza, graba y devuelve el índice nuevo; si es hoy, devuelve el que ya estaba. Es un `UPDATE` como máximo por persona por día, en una ruta que además ya escribe en cada request (el log de rate limit y `ensureUser`), así que no introduce una clase de comportamiento nueva. Efecto de borde conocido: con la pestaña abierta cruzando la medianoche el tip no cambia hasta recargar, que es exactamente lo que pide "estable durante la jornada".
  - *`tip_index` es un contador monótono, no una posición*: el backend lo incrementa sin saber cuántos tips hay, y el frontend resuelve `DAILY_TIPS[tip_index % DAILY_TIPS.length]`. Así el ciclado vive donde vive el catálogo, la base nunca necesita conocer su tamaño, y agregar tips después no rompe nada ni exige tocar el backend.
  - *Alternativa descartada*: `localStorage` con dos claves `tasking-tip-*`. Era el plan original: cero migración, cero endpoint, implementable en el día. Se descartó porque el avance por navegador reinicia la secuencia al cambiar de máquina o limpiar el almacenamiento, justo en una funcionalidad cuya premisa es recorrer los fundamentos en orden.
  - *Alternativa descartada*: mover también el catálogo a una tabla. Editar sin deploy solo sirve de verdad con una UI de administración que no está planificada; sin ella, corregir una coma sería escribir SQL en producción. Además suma una query por carga y el catálogo pierde su historia en git, donde hoy convive con `tips.md` y el registro de qué se aprobó y qué se descartó.
...
- **[Riesgo]** La capacidad pasa a depender de una migración de esquema y de un endpoint, así que ya no es un cambio puramente de frontend → **[Mitigación]** la migración es aditiva (dos columnas nullable en `users`, sin backfill ni cambio de contrato), va en rama + PR como pide CLAUDE.md, y si la escritura del avance falla el tip igual se muestra: el spec lo exige.
...
Cambio aditivo: markup y estilos nuevos, el catálogo estático en `public/tips.js`, una clave de `localStorage` para el tope del realce, y la migración `0014` que agrega `tip_index` y `tip_date` a `users` (ambas nullable, sin backfill: quien no tenga avance empieza por el primer tip). Sin cambios de contrato en endpoints existentes. Va en rama + PR por ser migración de esquema. Deploy por el flujo normal del proyecto (local → staging → aprobación → producción). Rollback = revertir el deploy; las columnas quedan inertes y la clave huérfana de `localStorage` también.
