...
- **Performance: cerrar la medición posterior a v2.2.0** — 🟢 esfuerzo chico y próximo paso recomendado. Repetir en producción la secuencia de `docs/PERFORMANCE-2026-09-11.md` con al menos cinco muestras por acción: crear y editar una tarjeta, asignar y quitar una etiqueta. Registrar tiempos del Worker y tiempo percibido desde clic hasta actualización visible. Comparar contra la línea base de 1,8–2,1 s por secuencia y confirmar que desaparecieron las tres lecturas globales que agregaban ~1,4–1,5 s. Actualizar el informe con resultados, condiciones y limitaciones.
...
- **Performance: instrumentar y reducir overhead D1 por petición** — 🟡 esfuerzo medio. La línea base mostró medianas de 353–614 ms con apenas 3–8 ms de CPU; incluso `/version` tardó 360 ms. Agregar tiempos por etapa y consulta para separar rate limiting, autenticación/revocación, `ensureUser`, `seedAdminIfNeeded` y handler. Si se confirma el costo, sacar la inicialización de usuario y rol del camino de cada request sin debilitar la revocación inmediata. Evaluar junto con “Rate limiting más granular” y el hallazgo técnico de overhead D1.
